...
const moduleMutation_S0436_5 = {
    Mutation: {
        mgrUpdate_S0436_5: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
            var tInput = { ...args.datas };

            if (tInput.CHK_SHINTS === '1') tInput.IMPORT_COST_PAID = 'SHINTS';
            else if (tInput.CHK_BVT === '1') tInput.IMPORT_COST_PAID = 'BVT';
            else if (tInput.CHK_ETP === '1') tInput.IMPORT_COST_PAID = 'ETP';

            var isLocal = tInput.SAVE_TYPE === 'LOCAL';
            if (!tInput.SAVE_TYPE) {
                isLocal =
            ...
            var baseCost = isLocal
                ? parseFloat(tInput.LOCAL_PAY_IMPORT_COST)
                : parseFloat(tInput.HQ_PAY_IMPORT_COST);

            /*
            var baseCurrCd = isLocal
                ? tInput.CURR_CD_LOCAL
                : tInput.CURR_CD;
            */

            var baseCurrCd = '';
            if (tInput.CURR_CD) baseCurrCd = tInput.CURR_CD; 
            else if(tInput.CURR_CD_LOCAL) baseCurrCd = tInput.CURR_CD_LOCAL; 

            var costColumnMst = isLocal ? 'import_cost_local' : 'import_cost';
            var costCurrColumn = isLocal
                ? 'import_cost_curr_local'
                : 'import_cost_curr';
            var costPaidColumn = isLocal
                ? 'import_cost_paid_local'
                : 'import_cost_paid';
            var costPaidValue = isLocal
                ? tInput.IMPORT_COST_PAID_LOCAL
                : tInput.IMPORT_COST_PAID;
            var costColumnMem = isLocal ? 'import_cost_local' : 'import_cost';
            var costType = isLocal ? 'LOCAL_COST' : 'HQ_COST';
            ...
            let tSQL99 = `
                ...
                set
                    ${costColumnMst} = '${baseCost}',
                    ${costCurrColumn} = '${baseCurrCd}',
                    ${costPaidColumn} = '${costPaidValue || ''}',
                    duty_cost = '${tInput.DUTY_COST || 0}'
                where
            ...
            for (tIdx1 = 0; tIdx1 < tRet1.length; tIdx1++) {
                ...
                var tShipCost =
                    parseFloat(baseCost) * (tWeight / tTotalWeight);

                let tSQL99 = `
                    update ksv_shipment_mem
                    set
                        ${costColumnMem} = '${tShipCost}'
                    where
                        shipment_cd = '${tInput.SHIPMENT_CD}'
            ...
            var tRemain1 = parseFloat(baseCost);
            var totalWeight1 = 0;
            ...
            tRet3_0.forEach((col, i) => {
                ...
                var tCost = parseFloat(baseCost) * parseFloat(col.WEIGHT) / totalWeight1;

                console.log(`${col.BUYER_CD} :  ${tCost} = ${baseCost} * ${col.WEIGHT} / ${totalWeight1} `)
                ...
                console.log(`Compute Amt(${parseFloat(baseCost)}: ${tCost} / ${tRemain1} / ${sumAmt} ===> ${tShipCost}/${tObj.buyer_cd} / ${parseFloat(col.WEIGHT) / totalWeight1} / ${tCost} `);

                tRet3.push(tObj);
            });

            if (tShipmentMst.ETD) tShipmentMst.ETD = '';
            var tIdx3 = 0;
            for (tIdx3 = 0; tIdx3 < tRet3.length; tIdx3++) {
                ...
                if (tShipmentMst.ETD)  tCostDate = tShipmentMst.ETD; 
                ...
                let tSQL99 = `
                    ...
                    values
                        (
                            ...
                            '',
                            '${tShipmentMst.BL_NO}',
                            '${tShipmentMst.REG_DATETIME.substring(0, 8)}'
                        )
                `;
                const tSQL99_1 = prisma.$queryRaw(Prisma.raw(tSQL99));
                tSQLArray.push(tSQL99_1);
...
export default moduleMutation_S0436_5;
