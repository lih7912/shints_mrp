...
const moduleQuery_S0513_2 = {
    Query: {
        mgrQueryS0513_2: async (_, args, contextValue) => {
        ...
        mgrQueryS0513_2_bak: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
            ...
            let sqlStr_bak = `
                SELECT
                    A1.*,
                    A2.BUYER_NAME,
                    isnull(A6.DOCU_NO, '') as DOCU_NO,
                    isnull(A6.BL_NO, '') as BL_NO,
                    isnull(A3.CD_NAME, '') as SHIP_MODE_N,
                    isnull(A4.CD_NAME, '') as DELIVERY_TYPE_N,
                    0.0 AS SHIP_PRICE,
                    '' AS BL_FILE,
                ...
                FROM
                    (
                        SELECT
                            ...
                            isnull(A.DELIVERY_TYPE, '') as DELIVERY_TYPE,
                            isnull(A.EXFACTORY, '') as EXFACTORY,
                            isnull(sum(C.AVR_PRICE * A.SHIP_CNT), 0) as SHIP_AMOUNT2,
                            isnull(sum(C.TOT_CNT), 0) as ORDER_QTY,
                        ...
                        FROM
                            ...
                            KCD_STYLE D
                        WHERE
                            C.ORDER_CD LIKE '%${args.data.ORDER_CD}%'
                            AND A.ORDER_CD = C.ORDER_CD
                            AND A.INVOICE_NO like '%${args.data.INVOICE_NO}%'
                            AND C.FACTORY_CD like '%${args.data.FACTORY_CD}%'
                            ...
                            AND (D.STYLE_CD like '%${args.data.STYLE_CD}%')
                            AND A.SHIP_DATE between '${sDate}' and '${eDate}'
                        GROUP BY
                            A.INVOICE_NO,
                            LEFT(C.ORDER_CD, 2),
                            A.SHIP_DATE,
                    ...
                    and A3.CD_GROUP = 'SHIP_PTYPE'
                    left join KSV_INVOICE_MST A6 ON A6.INVOICE_NO = A1.INVOICE_NO
                    left join KCD_CODE A4 ON A1.DELIVERY_TYPE = A4.CD_CODE
                    and A4.CD_GROUP = 'DELIVERY_TYPE',
                    KCD_BUYER A2
                where
                    A1.BUYER_CD = A2.BUYER_CD
                    -- order by A1.INVOICE_NO, A1.SHIP_DATE
                order by
                    A1.SHIP_DATE,
                    A1.INVOICE_NO
            `;
            ...
            let sqlStr = `
                SELECT
                    ...
                    isnull(F.ORD_AMT, '') as SHIP_AMOUNT,
                    -- isnull(A3.CD_NAME, '') as SHIP_MODE_N, 
                    -- isnull(A4.CD_NAME, '') as DELIVERY_TYPE_N ,
                    0.0 AS SHIP_PRICE,
                    '' AS BL_FILE,
                    ...
                    isnull(sum(B.TOT_CNT), 0) as ORDER_QTY,
                    isnull(sum(A.SHIP_CNT), 0) AS SHIP_QTY,
                    isnull(sum(A.SHIP_PRICE * A.SHIP_CNT), 0) as SHIP_AMOUNT3
                FROM
                    KSV_ORDER_MST D,
                    KSV_ORDER_MEM B,
                    KSV_PROD_MST C,
                ...
                where
                    ...
                    AND G.ORDER_CD LIKE '%${args.data.ORDER_CD}%'
                    AND F.INVOICE_NO like '%${args.data.INVOICE_NO}%'
                    AND D.FACTORY_CD like '%${args.data.FACTORY_CD}%'
                    AND E.BUYER_CD like '%${args.data.BUYER_CD}%'
                    AND (
                        D1.STYLE_CD like '%${args.data.STYLE_CD}%'
                        or D1.STYLE_NAME like '%${args.data.STYLE_CD}%'
                    )
                    AND F.SHIP_DATE between '${sDate}' and '${eDate}'
                GROUP BY
                    F.INVOICE_NO,
                    F.BUYER_CD,
                    F.SHIP_DATE,
                    G.NAT_CD,
                    -- A.SHIP_PTYPE, A.DELIVERY_TYPE, 
                    B1.BUYER_NAME,
                    F.DOCU_NO,
                    F.TOT_AMT,
                    F.ADJ_AMT,
                    F.ORD_AMT
                    --, A3.CD_NAME, A4.CD_NAME
                order by
                    A.SHIP_DATE,
                    A.INVOICE_NO
            `;
        ...
        mgrQueryS0513_2_bak2: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
            ...
            if (args.data.INVOICE_NO !== '' || args.data.ORDER_CD !== '') {
                ...
                tSQL1 = `    AND   F.SHIP_DATE between '${sDate}' and '${eDate}'`;
            }
        ...
        mgrQueryS0513_2_bak3: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
        ...
        mgrQueryS0513_2_2: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
...
export default moduleQuery_S0513_2;
