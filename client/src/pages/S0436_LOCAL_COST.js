...
import { ServiceS0436_LOCAL_COST } from "../service/service_biz/ServiceS0436_LOCAL_COST";
...
const emptyEDT_KSV_PO_MRP = {
    CURR_CD: "",
    CURR_CD_LOCAL: "",
    LOCAL_PAY_IMPORT_COST: "",
    LOCAL_PAY_IMPORT_COST_PAID: "",
    HQ_PAY_IMPORT_COST: "",
    HQ_PAY_IMPORT_COST_PAID: "",
    CHK_SHINTS: "",
    CHK_BVT: "",
    CHK_ETP: "",
    CHK_OTHER: "",
...
const S0436_LOCAL_COST = () => {
    ...
    const serviceS0436_LOCAL_COSTRef = useRef(null);
    if (!serviceS0436_LOCAL_COSTRef.current) serviceS0436_LOCAL_COSTRef.current = new ServiceS0436_LOCAL_COST();
    const serviceS0436_LOCAL_COST = serviceS0436_LOCAL_COSTRef.current;
    ...
    const search_LIST_1 = () => {
        ...
        serviceS0436_LOCAL_COST.mgrQuery_LIST_1(tObj).then((data) => {
    ...
    const process_UPDATE_IMPORT_COST = (type) => {
        if (!selectedTBL_KSV_PO_MRP || selectedTBL_KSV_PO_MRP.length === 0) {
            alert("Please select a shipment.");
            return;
        }

        const selected = selectedTBL_KSV_PO_MRP[0];

        let tInput = {
            SHIPMENT_CD: selected.SHIPMENT_CD,
            SAVE_TYPE: type,
            CURR_CD: dataEDT_KSV_PO_MRP.CURR_CD,
            CURR_CD_LOCAL: dataEDT_KSV_PO_MRP.CURR_CD_LOCAL,
            HQ_PAY_IMPORT_COST: "",
            LOCAL_PAY_IMPORT_COST: "",
            IMPORT_COST_PAID: "",
        ...
        if (type === "HQ") {
            if (!tInput.CURR_CD) {
                alert("Curr is a required field.");
                return;
            }

            if (!dataEDT_KSV_PO_MRP.HQ_PAY_IMPORT_COST) {
                alert("HQ Pay Import Cost is required.");
                return;
            }

            tInput.HQ_PAY_IMPORT_COST = dataEDT_KSV_PO_MRP.HQ_PAY_IMPORT_COST;

            if (dataEDT_KSV_PO_MRP.CHK_SHINTS === "1")
                tInput.IMPORT_COST_PAID = "SHINTS";
            else if (dataEDT_KSV_PO_MRP.CHK_BVT === "1")
                tInput.IMPORT_COST_PAID = "BVT";
            else if (dataEDT_KSV_PO_MRP.CHK_ETP === "1")
                tInput.IMPORT_COST_PAID = "ETP";
            else {
                alert("Please select Prepaid By.");
                return;
            }
        } else if (type === "LOCAL") {
            if (!tInput.CURR_CD_LOCAL) {
                alert("LOCAL Curr is a required field.");
                return;
            } else {
                tInput.CURR_CD = dataEDT_KSV_PO_MRP.CURR_CD_LOCAL;
            }

            delete tInput.CURR_CD_LOCAL;

            if (!dataEDT_KSV_PO_MRP.LOCAL_PAY_IMPORT_COST) {
                alert("Local Pay Import Cost is required.");
                return;
            }

            tInput.LOCAL_PAY_IMPORT_COST =
                dataEDT_KSV_PO_MRP.LOCAL_PAY_IMPORT_COST;

            if (dataEDT_KSV_PO_MRP.CHK_BVT_LOCAL === "1")
                tInput.IMPORT_COST_PAID_LOCAL = "BVT";
            else if (dataEDT_KSV_PO_MRP.CHK_ETP_LOCAL === "1")
                tInput.IMPORT_COST_PAID_LOCAL = "ETP";
            else {
                alert("Please select Prepaid By.");
                return;
            }
        ...
        serviceS0436_LOCAL_COST.mgrUpdate_IMPORT_COST(tInput).then((data) => {
            setLoadingTBL_KSV_PO_MRP(false);

            if (typeof data.graphQLErrors === "undefined") {
                if (data[0].CODE && data[0].CODE.includes("SUCC")) {
                    alert("Saved successfully.");
                    search_LIST_1();
                } else {
                    alert(data[0].CODE || "Update failed.");
                }
            } else {
                alert("Update failed.");
            }
    ...
    const onInputChangeQRY_KSV_PO_MRP_PAYER = (e, name) => {
    ...
    const onRowClick1TBL_KSV_PO_MRP = (argData0) => {
        ...
        const selectedCurrCd =
            argData.IMPORT_COST_CURR || argData.SHIPPING_COST_CURR || "";
        const selectedCurrCdLocal =
            argData.IMPORT_COST_CURR_LOCAL || argData.SHIPPING_COST_CURR || "";
        const selectedImportCostPaid =
            argData.IMPORT_COST_PAID || argData.SHIPPING_COST_PAID || "";
        const selectedImportCostPaidLocal =
            argData.IMPORT_COST_PAID_LOCAL || "";

        const importCostPaid = String(selectedImportCostPaid)
            .trim()
            .toUpperCase();
        const importCostPaidLocal = String(selectedImportCostPaidLocal)
            .trim()
            .toUpperCase();

        tObj.CURR_CD = selectedCurrCd;
        tObj.CURR_CD_LOCAL = selectedCurrCdLocal;
        tObj.HQ_PAY_IMPORT_COST = argData.IMPORT_COST ?? "";
        tObj.LOCAL_PAY_IMPORT_COST = argData.IMPORT_COST_LOCAL ?? "";
        tObj.IMPORT_COST_PAID = importCostPaid;
        tObj.IMPORT_COST_PAID_LOCAL = importCostPaidLocal;

        tObj.CHK_SHINTS = "0";
        tObj.CHK_BVT = "0";
        tObj.CHK_ETP = "0";
        tObj.CHK_OTHER = "0";
        tObj.CHK_BVT_LOCAL = "0";
        tObj.CHK_ETP_LOCAL = "0";
        if (tObj.IMPORT_COST_PAID === "SHINTS") tObj.CHK_SHINTS = "1";
        else if (tObj.IMPORT_COST_PAID === "BVT") tObj.CHK_BVT = "1";
        else if (tObj.IMPORT_COST_PAID === "ETP") tObj.CHK_ETP = "1";
        else if (tObj.IMPORT_COST_PAID !== "") tObj.CHK_OTHER = "1";

        if (tObj.IMPORT_COST_PAID_LOCAL === "BVT") tObj.CHK_BVT_LOCAL = "1";
        else if (tObj.IMPORT_COST_PAID_LOCAL === "ETP")
            tObj.CHK_ETP_LOCAL = "1";
        setDataEDT_KSV_PO_MRP(tObj);

        editEDT_KSV_PO_MRP_CURR_CD(tObj.CURR_CD);
        editEDT_KSV_PO_MRP_CURR_CD_LOCAL(tObj.CURR_CD_LOCAL);
    };
    ...
    const exportExcelTBL_KSV_PO_MRP = () => {
        ...
        datasTBL_KSV_PO_MRP.forEach((col, i) => {
            ...
            tObj.Payer = col.SHIPPING_COST_PAID;
            tObj.Curr_Import = col.IMPORT_COST_CURR;
            tObj.Import_Cost = col.IMPORT_COST;
            tObj.Payer_Import = col.IMPORT_COST_PAID;
            tObj.Duty_Cost = col.DUTY_COST;
            tObj.Customs_no = col.CUSTOMS_NO;
            tArray.push(tObj);
        });
        ...
        import("xlsx").then((xlsx) => {
            ...
            saveAsExcelFileTBL_KSV_PO_MRP(excelBuffer, "Local Cost List");
        });
    };
    ...
    const [dataEDT_KSV_PO_MRP_CURR_CD, setDataEDT_KSV_PO_MRP_CURR_CD] =
        useState({});
    const [
        dataEDT_KSV_PO_MRP_CURR_CD_LOCAL,
        setDataEDT_KSV_PO_MRP_CURR_CD_LOCAL,
    ] = useState({});
    ...
    const editEDT_KSV_PO_MRP_CURR_CD_LOCAL = (argValue) => {
        let _data = datasEDT_KSV_PO_MRP_CURR_CD.filter(
            (val) => val.CD_CODE === argValue,
        );
        setDataEDT_KSV_PO_MRP_CURR_CD_LOCAL(_data[0]);
    };
    ...
    const onDropdownChangeEDT_KSV_PO_MRP_CURR_CD_LOCAL = (e, name) => {
        let val = (e.value && e.value.CD_CODE) || "";
        ...
        setDataEDT_KSV_PO_MRP(_dataEDT_KSV_PO_MRP);
        setDataEDT_KSV_PO_MRP_CURR_CD_LOCAL(e.value);
    };

    const onInputChangeEDT_KSV_PO_MRP_IMPORT_COST = (e, name) => {
        let val = (e.target && e.target.value) || "";
    ...
    const onCheckboxChangeEDT_KSV_PO_MRP_CHK_BVT = (e, name) => {
        ...
        if (name === "CHK_BVT_LOCAL") {
            _dataEDT_KSV_PO_MRP.CHK_BVT_LOCAL = e.checked ? "1" : "0";

            if (e.checked) {
                _dataEDT_KSV_PO_MRP.CHK_ETP_LOCAL = "0";
            }
        } else {
            _dataEDT_KSV_PO_MRP[name] = e.checked ? "1" : "0";
        }
    ...
    const onCheckboxChangeEDT_KSV_PO_MRP_CHK_ETP = (e, name) => {
        ...
        if (name === "CHK_ETP_LOCAL") {
            _dataEDT_KSV_PO_MRP.CHK_ETP_LOCAL = e.checked ? "1" : "0";

            if (e.checked) {
                _dataEDT_KSV_PO_MRP.CHK_BVT_LOCAL = "0";
            }
        } else {
            _dataEDT_KSV_PO_MRP[name] = e.checked ? "1" : "0";
        }
    ...
    const [
        datasEDT_KSV_PO_MRP_CURR_CD_DUTY,
        setDatasEDT_KSV_PO_MRP_CURR_CD_DUTY,
    ] = useState([]);
    const [
        dataEDT_KSV_PO_MRP_CURR_CD_DUTY,
        setDataEDT_KSV_PO_MRP_CURR_CD_DUTY,
    ] = useState({});
    ...
    useEffect(() => {
        var tObj = {};
        serviceS0436_LOCAL_COST.mgrQuery_CODE(tObj).then((data) => {
            if (typeof data.graphQLErrors === "undefined") {
                ...
                setDataEDT_KSV_PO_MRP_CURR_CD(data.CURR_CD[10]); //USD

                setDataEDT_KSV_PO_MRP((prev) => ({
                    ...prev,
                    CHK_SHINTS: "1",
                    LOCAL_PAY_IMPORT_COST: "0",
                    HQ_PAY_IMPORT_COST: "0",
                }));

                var tRetDate = serviceLib.getCurrDate();
                var tQry = { ...dataQRY_KSV_PO_MRP };
                tQry.S_ETA = `${tRetDate.substring(0, 4)}0101`;
                tQry.E_ETA = `${tRetDate.substring(0, 4)}1231`;
    ...
    return (
        <div className="af-div-main">
            ...
            >
                ...
                <span className="af-span-3" style={{ width: "17rem" }}>
                    <p className="af-span-p" style={{ width: "7rem" }}>Prepaided by</p>
                    <div className="af-span-div" style={{ width: "9rem" }}>
                        <InputText
                            style={{ width: "9rem" }}
                            id="id_BL_FILE"
                            value={dataQRY_KSV_PO_MRP.PAYER}
                            onChange={(e) =>
                                onInputChangeQRY_KSV_PO_MRP_PAYER(e, "PAYER")
                            }
                        />
                    </div>
                </span>
            ...
            >
                ...
                >
                    ...
                    <AFColumn field="ETD" headerClassName="t-header" header="ETD" style={{ width: "6rem", flexBasis: "auto" }} body={(rowData) => serviceLib.dateFormat(rowData.ETD)} ></AFColumn>
                    ...
                    <AFColumn className="text-right" field="S_WEIGHT" headerClassName="t-header" header="Weight" style={{ width: "6rem", flexBasis: "auto" }} body={(rowData) => serviceLib.numWithCommas(rowData.S_WEIGHT, 4) } ></AFColumn>
                    <AFColumn className="text-right" field="S_CBM" headerClassName="t-header" header="CBM" style={{ width: "6rem", flexBasis: "auto" }} body={(rowData) => serviceLib.numWithCommas(rowData.S_CBM, 4) } ></AFColumn>
                    ...
                    <AFColumn className="text-right" field="SHIPPING_COST" headerClassName="t-header" header="Cost" style={{ width: "6rem", flexBasis: "auto" }} body={(rowData) => serviceLib.numWithCommas(rowData.SHIPPING_COST, 4) } ></AFColumn>
                    <AFColumn field="SHIPPING_COST_PAID" headerClassName="t-header" header="Payer" style={{ width: "6rem", flexBasis: "auto" }} ></AFColumn>
                    <AFColumn field="IMPORT_COST_CURR" headerClassName="t-header" header="Curr" style={{ width: "3rem", flexBasis: "auto" }} ></AFColumn>
                    <AFColumn className="text-right" field="IMPORT_COST" headerClassName="t-header" header="IM Cost(HQ)" style={{ width: "6rem", flexBasis: "auto" }} body={(rowData) => serviceLib.numWithCommas(rowData.IMPORT_COST, 4) } ></AFColumn>
                    <AFColumn field="IMPORT_COST_CURR_LOCAL" headerClassName="t-header" header="Curr" style={{ width: "3rem", flexBasis: "auto" }} ></AFColumn>
                    <AFColumn className="text-right" field="IMPORT_COST_LOCAL" headerClassName="t-header" header="IM Cost(Fac)" style={{ width: "6rem", flexBasis: "auto" }} body={(rowData) => serviceLib.numWithCommas( rowData.IMPORT_COST_LOCAL, 4, ) } ></AFColumn>

                    <AFColumn field="CUSTOMS_NO" headerClassName="t-header" header="Customs#" style={{ width: "10rem", flexBasis: "auto" }} ></AFColumn>
            ...
            >
                <div>
                    ...
                    <span className="af-span-3-0" style={{ width: "20rem" }}>
                        <p className="af-span-p" style={{ width: "10rem" }}>HQ pay Import Cost</p>
                        <div className="af-span-div" style={{ width: "9rem" }}>
                            <InputText
                                className="text-right"
                                style={{ width: "9rem" }}
                                id="id_BL_FILE"
                                value={dataEDT_KSV_PO_MRP.HQ_PAY_IMPORT_COST}
                                onChange={(e) =>
                                    onInputChangeEDT_KSV_PO_MRP_IMPORT_COST(
                                        e,
                                        "HQ_PAY_IMPORT_COST",
                                    )
                                }
                    ...
                    <span className="af-span-3-0" style={{ width: "8rem" }}>
                        <p className="af-span-p" style={{ width: "9rem" }}>Prepaid By:</p>
                    </span>
                    ...
                    <span className="af-span-3-0" style={{ width: "11rem" }}>
                        ...
                        >
                            <Button
                                ...
                                label="Save HQ"
                                className="p-button-text"
                                onClick={() => process_UPDATE_IMPORT_COST("HQ")}
                            />
                        </div>
                    </span>
                </div>
                ...
                <div>
                    <span className="af-span-3-0" style={{ width: "17rem" }}>
                        ...
                        <div className="af-span-div" style={{ width: "9rem" }}>
                            <Dropdown
                                style={{ width: "7rem" }}
                                value={dataEDT_KSV_PO_MRP_CURR_CD_LOCAL}
                                onChange={(e) =>
                                    onDropdownChangeEDT_KSV_PO_MRP_CURR_CD_LOCAL(
                                        e,
                                        "CURR_CD_LOCAL",
                                    )
                                }
                    ...
                    <span className="af-span-3-0" style={{ width: "20rem" }}>
                        <p className="af-span-p" style={{ width: "10rem" }}>Local pay Import Cost</p>
                        <div className="af-span-div" style={{ width: "9rem" }}>
                            <InputText
                                className="text-right"
                                style={{ width: "9rem" }}
                                value={dataEDT_KSV_PO_MRP.LOCAL_PAY_IMPORT_COST}
                                onChange={(e) =>
                                    onInputChangeEDT_KSV_PO_MRP_IMPORT_COST(
                                        e,
                                        "LOCAL_PAY_IMPORT_COST",
                    ...
                    <span className="af-span-3-0" style={{ width: "6rem" }}>
                        ...
                        <div className="af-span-checkbox">
                            <Checkbox
                                checked={changeCheckBoxVal(
                                    dataEDT_KSV_PO_MRP.CHK_BVT_LOCAL,
                                )}
                                onChange={(e) =>
                                    onCheckboxChangeEDT_KSV_PO_MRP_CHK_BVT(
                                        e,
                    ...
                    <span className="af-span-3-0" style={{ width: "6rem" }}>
                        ...
                        <div className="af-span-checkbox">
                            <Checkbox
                                checked={changeCheckBoxVal(
                                    dataEDT_KSV_PO_MRP.CHK_ETP_LOCAL,
                                )}
                                onChange={(e) =>
                                    onCheckboxChangeEDT_KSV_PO_MRP_CHK_ETP(
                                        e,
                    ...
                    <span className="af-span-3-0" style={{ width: "11rem" }}>
                        ...
                        >
                            <Button
                                ...
                                className="p-button-text"
                                onClick={() =>
                                    process_UPDATE_IMPORT_COST("LOCAL")
                                }
                            />
                        </div>
                    </span>
...
export default React.memo(S0436_LOCAL_COST, comparisonFn);
