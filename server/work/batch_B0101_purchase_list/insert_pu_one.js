...
const process1 = async (argPoCd, argVendorCd) => {
    ...
    `;
    console.log(tInPuMst2Str);

    var tArray = [...tInPuMst2Str];
    ...
    var tQryPoWorkList = await prisma.$queryRaw`
        ...
        where
            po_cd = ${argPoCd}
    `;
    if (tQryPoWorkList.length <= 0) {
        var tInPoWorkList = await prisma.$queryRaw`
            insert into
                ksv_po_worklist (
                    po_cd,
                    status_cd,
    ...
    try {
        ...
        console.log(
            `Make Purchase  Error: ${argPoCd} / ${argVendorCd}/ ${e.message} `,
        );
        return 'FAIL';
    }
};
    ...
    for (tIdx = 0; tIdx < tRet0.length; tIdx++) {
        ...
        if (tRet1.length > 0 && tRet3[0].cnt > 0) {
            ...
            for (tIdx1 = 0; tIdx1 < tRet1.length; tIdx1++) {
                ...
                if (tRet4.length > 0) {
                    console.log(
                        `>>>>(2): ${tOne.po_cd}|${tOne2.VENDOR_CD}| Already Exist| ${tRet4[0].pu_cd} `,
                    );
                    continue;
                }
