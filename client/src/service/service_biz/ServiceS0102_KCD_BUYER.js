...
export class ServiceS0102_KCD_BUYER {
    //
    ...
    async mgr1KcdStyleKsvProdMst(argStyleCd) {
        ...
        try {
            const { loading, error, data } = await client.query({
                query: gql`
                    query Mgr1KcdStyleKsvProdMst($styleCd: String!) {
                        mgr1KcdStyleKsvProdMst(STYLE_CD: $styleCd) {
                            id
                            PROD_CD
                            STYLE_CD
                            PROD_TYPE
            ...
            console.log(
                "MGR1_KCD_BANK_CODE:" + data.mgr1KcdStyleKsvProdMst.length,
            );
            return data.mgr1KcdStyleKsvProdMst;
        } catch (e) {
            return e;
        }
    }
    ...
    async mgr1KcdStyle(argStyleName, argBuyerName, argKind) {
        ...
        try {
            ...
            console.log("MGR1_KCD_BANK_CODE:" + data.mgr1KcdStyle.length);
            return data.mgr1KcdStyle;
        } catch (e) {
            return e;
        }
    }
    ...
    async mgr1KcdStyleChild(argStyleCd) {
        ...
        try {
            const { loading, error, data } = await client.query({
                query: gql`
                    query Mgr1KcdStyleChild($styleCd: String!) {
                        mgr1KcdStyleChild(STYLE_CD: $styleCd) {
                            id
                            STYLE_CD
                            STYLE_NAME
                            BUYER_CD
            ...
            console.log("MGR1_KCD_BANK_CODE:" + data.mgr1KcdStyleChild.length);
            return data.mgr1KcdStyleChild;
        } catch (e) {
            return e;
        }
    }
    ...
    async mgr1KcdStyleCode(argKind1) {
        ...
        try {
            const { loading, error, data } = await client.query({
                query: gql`
                    query Mgr1KcdStyleCode($kind1: String!) {
                        mgr1KcdStyleCode(KIND1: $kind1) {
            ...
            console.log(
                "MGR1_KCD_BANK_CODE(STATUS_CD):" +
                    data.mgr1KcdStyleCode.T_KCD_STYLE_STATUS_CD.length,
            );
            console.log(
                JSON.stringify(data.mgr1KcdStyleCode.T_KCD_STYLE_STATUS_CD),
            );
            console.log(
                "MGR1_KCD_BANK_CODE(STYLE_DL):" +
                    data.mgr1KcdStyleCode.T_KCD_STYLE_DL.length,
            );
            console.log(JSON.stringify(data.mgr1KcdStyleCode.T_KCD_STYLE_DL));

            return data.mgr1KcdStyleCode;
        } catch (e) {
            return e;
        }
    }
    ...
    async mgr1KcdStyleSave(argsKCD_STYLE) {
        ...
        try {
            const { loading, error, data } = await client.mutate({
                mutation: gql`
                    mutation Mgr1KcdStyleSave($datas: TMgr1KCD_STYLE_IN!) {
                        mgr1KcdStyleSave(datas: $datas)
                    }
                `,
                variables: {
                    datas: {
            ...
            return data.mgr1KcdStyleSave;
        } catch (e) {
            return e;
        }
    }

    async mgr1KsvProdMstSave(argsKsvProdMst, argsBuyerCd) {
        apolloOption.cache = new InMemoryCache();
        const client = new ApolloClient(apolloOption);
        ...
        try {
            const { loading, error, data } = await client.mutate({
                mutation: gql`
                    mutation Mgr1KsvProdMstSave(
                        $datas: [I_KSV_PROD_MST!]!
                        $buyerCd: String!
                    ) {
                        mgr1KsvProdMstSave(datas: $datas, BUYER_CD: $buyerCd)
                    }
                `,
                variables: {
            ...
            return data.mgr1KsvProdMstSave;
        } catch (e) {
            return e;
        }
    }
