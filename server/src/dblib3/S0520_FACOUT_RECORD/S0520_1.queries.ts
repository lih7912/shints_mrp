...
const moduleQuery_S0520_1 = {
    Query: {
        mgrQueryS0520_1_CODE: async (_, args, contextValue) => {
            ...
            console.log(args.data.PO_CD, args.data.ORDER_CD);
            ...
            if (args.data.PO_CD !== '') {
                let tRet = [];
                let tObj = {};
                tObj.CD_CODE = args.data.PO_CD;
                tObj.CD_NAME = args.data.PO_CD;
                tRet.unshift(tObj);
                tWObj.PO_CD = tRet;
            } else {
                let tRet = [];
...
*/
        ...
        mgrQueryS0520_1_CODE_DETAIL: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
            ...
            if (args.data.PO_CD !== '') {
                ...
                let buyerCdResult = await prisma.$queryRaw(
                    Prisma.raw(
                        `
                            select
                                top 1 left(ORDER_CD, 2) as BUYER_CD
                            from
                                ksv_po_mem
                            where
                                po_cd = '${args.data.PO_CD}'
                ...
                if (buyerCdResult.length) {
                    let buyerCd = buyerCdResult[0].BUYER_CD;
                    tWObj.BUYER_CD = [
                        { BUYER_CD: buyerCd, BUYER_NAME: buyerCd },
                    ];
                }

                let orderCdResult = await prisma.$queryRaw(
                    Prisma.raw(
                        `
                            select distinct
                                ORDER_CD
                ...
                if (orderCdResult.length) {
                    orderCdResult.unshift({ ORDER_CD: '', ORDER_NAME: ' ' });
                    tWObj.ORDER_CD = orderCdResult.map((col) => ({
                        CD_CODE: col.ORDER_CD,
                        CD_NAME: col.ORDER_CD,
                    }));
                    console.log('Order CD Result:', tWObj.ORDER_CD);
                }
            } else {
                tWObj.PO_CD = [];
            }
        ...
        mgrQueryS0520_1_CODE_DETAIL2: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
        ...
        mgrQueryS0520_1_CODE0: async (_, args) => {
            var tWObj = {};
        ...
        mgrQueryS0520_1_CODE2: async (_, args) => {
            var tWObj = {};
...
export default moduleQuery_S0520_1;
