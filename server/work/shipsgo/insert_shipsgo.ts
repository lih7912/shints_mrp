...
const process1 = async (argPuCd: string) => {
    ...
    var sqlBL_NO: string = `
        select
            isnull(bl_no, '') as bl_no,
            isnull(ship_line, '') as ship_line,
            isnull(tracking_id, '') as tracking_id
        from
            ksv_blno_mst
    `;
    ...
    for (tIdx = 0; tIdx < objBL_NO.length; tIdx++) {
        ...
        console.log(`=======================================================`);
        console.log(
            `>>>>> (1) ${tOne['bl_no']} | ${tOne['ship_line']} | ${tOne['tracking_id']} `,
        );
        ...
        if (tTrackingId === '') {
            ...
            myJSONObject['authCode'] = 'fe73641fc4eddff2f0239273bb1b3a3c';
            ...
            if (tRequestId !== '') {
                var sqlUpdate: string = `
                    update ksv_blno_mst
                    set
                        tracking_id = '${tRequestId}'
                    where
                        bl_no = '${tBlNo}'
                        and ship_line = '${tShipLine}'
                `;
                var objUpdate: any = await prisma.$queryRaw(
                    Prisma.raw(sqlUpdate),
                );
                console.log(
                    `>>>>> (21) ${tBlNo} | ${tShipLine} | ${tRequestId} `,
                );
            }
        } else {
            var tUrl = `https://shipsgo.com/api/v1.1/ContainerService/GetContainerInfo/?authCode=fe73641fc4eddff2f0239273bb1b3a3c&requestId=${tTrackingId}&mapPoint=true`;
            ...
            console.log(
                `>>>>> (3) ${tBlNo} | ${tShipLine} | ${tTrackingId} | ${tStr} `,
            );
            console.log(
                `>>>>> (30) ${tA_ETA} | ${tF_ETA} | ${tSHIP_STATUS} | ${tLOADING_DATE} `,
            );
            console.log(
                `>>>>> (30) ${tDEPARTURE_DATE} | ${tARRIVAL_DATE} | ${tDISCHARGE_DATE} | ${tRetDate} `,
            );
            ...
            console.log(
                `>>>>> (31) ${tBlNo} | ${tShipLine} | ${tTrackingId} | ${tStr} `,
            );
        }
    }
};
