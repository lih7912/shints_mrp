...
const moduleQuery_S0512_1 = {
    Query: {
        mgrQueryS0512_CODE: async (_, args) => {
            var tWObj = {};
            ...
            let sqlStr = `
                ...
                FROM
                    KCD_FACTORY_WARE
                    -- WHERE FACTORY_CD = '${args.data.FACTORY_CD}' 
                WHERE
                    STATUS_CD = '0'
                ORDER BY
                    FACTORY_CD
            ...
            let sqlStr = `
                ...
                WHERE
                    STATUS_CD = '0'
                    and factory_cd in ('FC010', 'FC044', 'FC043')
                ORDER BY
                    FACTORY_NAME
            `;
            let tRet = await prisma.$queryRaw(Prisma.raw(sqlStr));
            ...
            tObj.CD_NAME = ' ';
            tRet.unshift(tObj);
            tWObj.REASON_REMARK = tRet;

            return tWObj;
        },
        mgrQueryS0512_1: async (_, args) => {
            let sqlStr = `
                SELECT DISTINCT
                    A.PO_CD
                FROM
...
export default moduleQuery_S0512_1;
