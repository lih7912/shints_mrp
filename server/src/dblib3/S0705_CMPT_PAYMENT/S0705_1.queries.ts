...
const moduleQuery_S0705_1 = {
    Query: {
        mgrQueryS0705_CODE: async (_, args) => {
            var tWObj = {};
            ...
            let sqlStr = `
                ...
                from
                    kcd_code
                where
                    cd_group = 'CMPT_SALES_PART'
                order by
                    cd_flag
            `;
            let tRet = await prisma.$queryRaw(Prisma.raw(sqlStr));
            var tObj = {};
            tWObj.INVOICE_TYPE = tRet;

            return tWObj;
        },
        mgrQueryS0705_1: async (_, args) => {
            let sqlStr = `
                SELECT DISTINCT
                    A.PO_CD
                FROM
...
export default moduleQuery_S0705_1;
