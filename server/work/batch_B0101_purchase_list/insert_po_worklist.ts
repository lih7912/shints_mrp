...
const process1 = async (
    ...
    var tObjPoMst: Object = { ...tObjPoMsts[9] };
    ...
    var tOrderDate: string = tObjPoMst['PO_CONF_DATE'];
...
const process0 = async () => {
    ...
    for (tIdx = 0; tIdx < tObjPoMsts.length; tIdx++) {
        ...
        `;
        var tObjPoMems: Object[] = await prisma.$queryRaw(Prisma.raw(sqlPoMem));
        if (tObjPoMems.length <= 0) continue;
        var tBuyerCd = tObjPoMems[0]['order_cd'].substring(0, 2);
        ...
        if (objPoWorkLists.length <= 0) {
            ...
            `;
            var objInPoWorkList: any = await prisma.$queryRaw(
                Prisma.raw(sqlInPoWorkList),
            );
            console.log(`=====>${tOne['po_cd']}`);
        }
    }
};
