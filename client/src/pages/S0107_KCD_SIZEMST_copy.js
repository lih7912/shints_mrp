...
const MgrKcdSizeMst = () => {
    ...
    const [datasKCD_SIZE_MST_STATUS_CD, setDatasKCD_SIZE_MST_STATUS_CD] = useState([]);
    const [dataKCD_SIZE_MST_STATUS_CD, setDataKCD_SIZE_MST_STATUS_CD] = useState({});

    const [datasKCD_SIZE_MST_BUYER_CD, setDatasKCD_SIZE_MST_BUYER_CD] = useState([]);
    ...
    const [dataKCD_SIZE_MST, setDataKCD_SIZE_MST] = useState(EMPTY_KCD_SIZE_MST);
    ...
    const updateSizeMstField = (field, value, { removeSpaces = false, updateCount = false } = {}) => {
        const nextValue = typeof value === 'string' && removeSpaces ? value.replaceAll(' ', '') : value;

        setDataKCD_SIZE_MST(prev => {
            const next = { ...prev, [field]: nextValue };

            if (updateCount) {
                next.SIZE_CNT = nextValue === '' ? 0 : String(nextValue).split(',').length;
            }
    ...
    const handleDataInputChange = (field, options = {}) => (e) => {
        const rawValue = (e.target && e.target.value) || '';

        if (field === 'SIZE_CNT') {
            updateSizeMstField(field, Number.parseInt(rawValue, 10) || 0, options);
            return;
        }
    ...
    const handleQueryInputChange = (setter, { removeSpaces = false } = {}) => (e) => {
    ...
    const validateSizeMember = () => {
        ...
        if (normalizedSizeMember.includes('xx')) {
            alert('XX는 사용할 수 없습니다.');
            return false;
        }

        if (hasConsecutiveCommas(compactSizeMember)) {
            alert('비어있는 SIZE(연속된 COMMA)가 있습니다.');
            return false;
        }

        const sizeArr = compactSizeMember.toLowerCase().split(',').filter(Boolean);
        const setSize = new Set(sizeArr);

        if (sizeArr.length !== setSize.size) {
            alert('중복된 사이즈는 입력할 수 없습니다.');
            return false;
        }
    ...
    const buildSearchInput = () => ({
        ...
        BUYER_CD:
            typeof dataQryBuyerCd?.BUYER_CD === 'undefined' || dataQryBuyerCd?.BUYER_CD === ' '
                ? ''
    ...
    const executeMutation = ({ serviceCall, successSummary, errorSummary, removeBuyerName = false }) => {
        ...
        serviceCall(payload).then((data) => {
            ...
            console.log(`ServiceMgrKCD_VENDOR.mgr1KcdBuyer error => ${JSON.stringify(data.graphQLErrors)}`);
            toast.current.show({ severity: 'error', summary: errorSummary, detail: '', life: 3000 });
        });
    };

    const process_INSERT_SIZEMST = () => {
        executeMutation({
            serviceCall: serviceS0107_KCD_SIZEMST.mgrInsert_INSERT_SIZEMST.bind(serviceS0107_KCD_SIZEMST),
    ...
    const process_UPDATE_SIZEMST = () => {
        executeMutation({
            serviceCall: serviceS0107_KCD_SIZEMST.mgrInsert_UPDATE_SIZEMST.bind(serviceS0107_KCD_SIZEMST),
            successSummary: 'ALERT',
    ...
    const process_DELETE_SIZEMST = () => {
        executeMutation({
            serviceCall: serviceS0107_KCD_SIZEMST.mgrInsert_DELETE_SIZEMST.bind(serviceS0107_KCD_SIZEMST),
    ...
    const editKCD_SIZE_MST = (row) => {
        ...
        const selectedStatus = datasKCD_SIZE_MST_STATUS_CD.find((item) => item.CD_CODE === row.STATUS_CD);
        setDataKCD_SIZE_MST_STATUS_CD(selectedStatus || {});

        const selectedBuyer = datasKCD_SIZE_MST_BUYER_CD.find((item) => item.BUYER_CD === row.BUYER_CD);
    ...
    const queryStatusValue = datasQryStatus.find((item) => item.CD_CODE === qrySizeStatusCD) || null;
    ...
    return (
        <div className="af-div-main">
            <div className="af-div-first" style={{ width: '123rem', height: '3rem' }}>
                <span className="af-span-3-0" style={{ width: '19rem' }}>
                    ...
                    <div className="af-span-div" style={{ width: '11rem' }}>
                        <Dropdown
                            ...
                            onChange={(e) => onDropdownChangeKCD_SIZE_MST_STATUS_CD(e, ' ')}
                ...
                <span className="af-span-3-0" style={{ width: '22rem' }}>
                    ...
                    <div className="af-span-div" style={{ width: '12rem' }}>
                        <InputText
                            style={{ width: '12rem' }}
                            type="search"
                            onChange={handleQueryInputChange(setDataQrySizeGroup, { removeSpaces: true })}
                            placeholder=""
                ...
                <span className="af-span-3-0" style={{ width: '31rem' }}>
                    ...
                    <div className="af-span-div" style={{ width: '21rem' }}>
                        <InputText
                            ...
                            type="search"
                            placeholder=""
                            onChange={handleQueryInputChange(setQrySizeMember, { removeSpaces: true })}
                            onKeyPress={handleSearchOnEnter}
                        />
                    </div>
                </span>
                ...
                <span className="af-span-3-0" style={{ width: '6rem' }}>
                    <div className="af-span-div-btn" style={{ width: '5rem' }}>
                        <Button style={{ width: '5rem' }} label="Excel" className="p-button-text green" onClick={exportExcel} />
                    </div>
                </span>
            </div>

            <div className="af-div-first" style={{ width: '123rem', height: '38rem' }}>
            ...
            <div className="af-div-first" style={{ width: '123rem', height: '20rem' }}>
                ...
                <span className="af-span-3-0" style={{ width: '120rem' }}>
                    <p className="af-span-p" style={{ width: '9rem', color: 'red' }}>*Buyer#</p>
                    <div className="af-span-div" style={{ width: '23rem' }}>
                        <Dropdown
                            style={{ width: '23rem' }}
                            id="id_STATUS_CD"
                            value={dataKCD_SIZE_MST_BUYER_CD}
                            onChange={(e) => onDropdownChangeKCD_SIZE_MST_BUYER_CD(e, 'BUYER_CD')}
                            options={datasKCD_SIZE_MST_BUYER_CD}
                            optionLabel="BUYER_NAME"
                            placeholder=""
                            filter
                ...
                <span className="af-span-3-0" style={{ width: '120rem' }}>
                    <p className="af-span-p" style={{ width: '9rem', color: 'red' }}>*Size Group</p>
                    <div className="af-span-div" style={{ width: '23rem' }}>
                        <InputText
                            style={{ width: '23rem' }}
                            id="id_SIZE_GROUP"
                            value={dataKCD_SIZE_MST.SIZE_GROUP_NAME}
                            onChange={handleDataInputChange('SIZE_GROUP_NAME', { removeSpaces: true })}
                        />
                    </div>
                </span>
                <span className="af-span-3-0" style={{ width: '120rem' }}>
                    <p className="af-span-p" style={{ width: '9rem', color: 'red' }}>*Size Content</p>
                    <div className="af-span-div" style={{ width: '71rem' }}>
                        <InputText
                            ...
                            onChange={handleDataInputChange('SIZE_MEMBER', { removeSpaces: true, updateCount: true })}
                        />
                    </div>
                </span>
                <span className="af-span-3-0" style={{ width: '120rem' }}>
                ...
                <span className="af-span-3-0" style={{ width: '120rem' }}>
                    ...
                    <div className="af-span-div" style={{ width: '10rem' }}>
                        <Dropdown
                            ...
                            onChange={(e) => onDropdownChangeKCD_SIZE_MST_STATUS_CD(e, 'STATUS_CD')}
                ...
                <span className="af-span-3-0" style={{ width: '6rem' }}>
                    <div className="af-span-div-btn" style={{ width: '5rem' }}>
                        <Button style={{ width: '5rem' }} label="Insert" className="p-button-text" onClick={process_INSERT} />
                    </div>
                </span>
                <span className="af-span-3-0" style={{ width: '6rem' }}>
                    <div className="af-span-div-btn" style={{ width: '5rem' }}>
                        <Button style={{ width: '5rem' }} label="Update" className="p-button-text" onClick={process_UPDATE} />
                    </div>
                </span>
                <span className="af-span-3-0" style={{ width: '6rem' }}>
                    <div className="af-span-div-btn" style={{ width: '5rem' }}>
                        <Button style={{ width: '5rem' }} label="Reset" className="p-button-text" onClick={resetKCD_SIZE_MST} />
                    </div>
                </span>
                <span className="af-span-3-0" style={{ width: '6rem' }}>
                    <div className="af-span-div-btn" style={{ width: '5rem' }}>
                        <Button style={{ width: '5rem' }} label="Delete" className="p-button-text" onClick={process_DELETE_SIZEMST} />
                    </div>
                </span>
            </div>
            <Toast ref={toast} />
