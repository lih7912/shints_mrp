...
const moduleMutation_S041206_5 = {
    Mutation: {
        mgrInsert_S041206_5: async (_, args, contextValue) => {
            //
...
*/
            ...
            args.datas.forEach((col, i) => {
                let tSQL99 = `
                    update ksv_stock_out_freight
                    set
                        delay_reason = '${col.DELAY_REASON}',
                        charge_kind = '${col.CHARGE_KIND}',
                        part = '${col.BUYER_TEAM}'
                    where
                        pack_cd = '${col.PACK_CD}'
                        and buyer_cd = '${col.BUYER_CD}'
                `;
            ...
            try {
                ...
                var tRetArray = [];
                var tObj = {};
                tObj.CODE = 'ERROR:Update Sea Freight Error';
                tObj.id = 0;
                tRetArray.push(tObj);
                return tRetArray;
            }
            ...
            var tRetArray = [];
            var tObj = {};
            tObj.CODE = 'SUCCEED:Update Sea Freight :' + args.datas.length;
            tObj.id = 0;
            tRetArray.push(tObj);
        ...
        mgrInsert_S041206_5_0: async (_, args, contextValue) => {
            //
            var tDateNew = new Date();
            tDateNew.setMonth(tDateNew.getMonth() + 1);
            var tZeroDate = '00';
...
*/
            ...
            let tSQL99 = `
                ...
                select
                    ...
                    '22',
                    case
                        when left(a.order_cd, 2) = 'SB' then '4'
                        when left(a.order_cd, 2) = 'EP' then '4'
                        else '3'
                    end,
                    c.cd_code,
                    round(sum(a.out_qty * f.pay_price * g.usd_rate), 2, 0),
                    'USD',
                    '0',
                    'N',
                    'N',
                ...
                from
                    ...
                    kcd_currency g
                where
                    a.pack_cd = '${tInput.PACK_CD}'
                    and left(a.order_cd, 2) = e.buyer_cd
                    and e.buyer_team = c.cd_code
                    and c.cd_group = 'buyer_team'
                    and a.po_cd = f.po_cd
                    and a.po_seq = f.po_seq
                    and a.order_cd = f.order_cd
                    and a.matl_cd = f.matl_cd
                    and a.mrp_seq = f.mrp_seq
                    --and g.start_date=left(f.in_datetime,8) 
                    and g.curr_cd = f.pay_curr_cd
                group by
                    left(a.order_cd, 2),
                    c.cd_code,
                    c.cd_flag
...
export default moduleMutation_S041206_5;
