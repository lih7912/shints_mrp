...
const moduleMutation_S0411_5 = {
    Mutation: {
        mgrInsert_S0411_5: async (_, args, contextValue) => {
            //
...
*/
            ...
            for (tIdx = 0; tIdx < args.datas.length; tIdx++) {
                ...
                let tSQL99 = `
                    ...
                    set
                        out_qty = out_qty + ${tInputOne.OUT_QTY},
                        stock_status = '2'
                    where
                ...
                let tSQL99 = `
                    ...
                    where
                        ...
                        and matl_cd = '${tInputOne.MATL_CD}'
                        and mrp_seq = ${tInputOne.MRP_SEQ}
                        and in_datetime = '${tInputOne.IN_DATE}'
                `;
                const tSQL99_1 = prisma.$queryRaw(Prisma.raw(tSQL99));
                tSQLArray.push(tSQL99_1);

                var tFactoryCd = '';
                if (tInput1.TO_TYPE === '1')
                    tFactoryCd = 'FC010'; // SHINTS
                else if (tInput1.TO_TYPE === '2')
                    tFactoryCd = 'FC034'; //BVT
                else if (tInput1.TO_TYPE === '3')
                    tFactoryCd = 'FC044'; // ETP
                else if (tInput1.TO_TYPE === '4')
                    tFactoryCd = 'FC087'; // TN
                else if (tInput1.TO_TYPE === '5') {
                    // Others
                    let tSql0 = `
                        select
                            out_factory_cd
                    ...
                    if (nRet0.length > 0) tFactoryCd = nRet0[0].out_factory_cd;
                }
                ...
                var m_Amount = 0;
                var m_BLNo = 0;
                var m_BuyerCd = tInputOne.substring(0, 2);
                if (
                    tInput1.DELIVERY_TYPE === '7' ||
                    tInput1.DELIVERY_TYPE === '6' ||
                    tInput1.DELIVERY_TYPE === 'D' ||
           ...
           `;
                ...
                let tSQL99 = `
                    ...
                    set
                        out_qty = out_qty + ${tInputOne.OUT_QTY},
                        remain_qty = remain_qty - ${tInputOne.OUT_QTY}
                    where
                        po_cd = '${tInputOne.PO_CD}'
                        and matl_cd = '${tInputOne.MATL_CD}'
                `;
                ...
                `;
                var nRet0 = await prisma.$queryRaw(Prisma.raw(tSql0));
                if (nRet0.length > 0 && nRet0[0].remain_qty <= 0) {
                    let tSQL99 = `
                        update kzz_delay_action
                        set
                            action_flag = 'Y'
...
export default moduleMutation_S0411_5;
