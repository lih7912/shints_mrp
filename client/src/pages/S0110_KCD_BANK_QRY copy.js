...
const MgrKcdBank = () => {
    ...
    const serviceS0110_KCD_BANK_QRY = new ServiceS0110_KCD_BANK_QRY();
    ...
    const [dataQRY_KCD_BANK_STATUS_CD, setDataQRY_KCD_BANK_STATUS_CD] =
        useState({});
    const editQRY_KCD_BANK_STATUS_CD = (argValue) => {
        let tObj = datasQRY_KCD_BANK_STATUS_CD.filter(
            (val) => val.CD_CODE === argValue,
        );
        setDataQRY_KCD_BANK_STATUS_CD(tObj[0]);
    };
    ...
    const [dataQRY_KCD_BANK_BANK_TYPE, setDataQRY_KCD_BANK_BANK_TYPE] =
        useState({});
    const editQRY_KCD_BANK_BANK_TYPE = (argValue) => {
        let tObj = datasQRY_KCD_BANK_BANK_TYPE.filter(
            (val) => val.CD_CODE === argValue,
        );
        setDataQRY_KCD_BANK_BANK_TYPE(tObj[0]);
    };
    ...
    const get_dataQRY_KCD_BANK = () => {
        var tObj = { ...dataQRY_KCD_BANK };
        tObj.BANK_NAME = dataQRY_KCD_BANK_BANK_NAME;
        tObj.ACCOUNT_NAME = dataQRY_KCD_BANK_ACCOUNT_NAME;
        tObj.STATUS_CD = dataQRY_KCD_BANK_STATUS_CD.CD_CODE;
        tObj.BANK_TYPE = dataQRY_KCD_BANK_BANK_TYPE.CD_CODE;
        return tObj;
    };
    const put_dataQRY_KCD_BANK = (argData) => {
        ...
        editQRY_KCD_BANK_STATUS_CD(argData.STATUS_CD);
        editQRY_KCD_BANK_BANK_TYPE(argData.BANK_TYPE);
    };
    ...
    const [dataEDT_KCD_BANK_BANK_TYPE1, setDataEDT_KCD_BANK_BANK_TYPE1] =
        useState({});
    const editEDT_KCD_BANK_BANK_TYPE1 = (argValue) => {
        let tObj = datasEDT_KCD_BANK_BANK_TYPE1.filter(
            (val) => val.CD_CODE === argValue,
        );
        setDataEDT_KCD_BANK_BANK_TYPE1(tObj[0]);
    };
    ...
    const [dataEDT_KCD_BANK_STATUS_CD, setDataEDT_KCD_BANK_STATUS_CD] =
        useState({});
    const editEDT_KCD_BANK_STATUS_CD = (argValue) => {
        let tObj = datasEDT_KCD_BANK_STATUS_CD.filter(
            (val) => val.CD_CODE === argValue,
        );
        setDataEDT_KCD_BANK_STATUS_CD(tObj[0]);
    };
    ...
    const get_dataEDT_KCD_BANK = () => {
        var tObj = { ...dataEDT_KCD_BANK };
        tObj.BANK_CD = dataEDT_KCD_BANK_BANK_CD;
        tObj.BANK_NAME = dataEDT_KCD_BANK_BANK_NAME;
        tObj.ACCOUNT_NAME = dataEDT_KCD_BANK_ACCOUNT_NAME;
        tObj.ACCOUNT_NO = dataEDT_KCD_BANK_ACCOUNT_NO;
        tObj.BANK_TYPE1 = dataEDT_KCD_BANK_BANK_TYPE1.CD_CODE;
        tObj.BANK_BRANCH = dataEDT_KCD_BANK_BANK_BRANCH;
        tObj.SFTCODE = dataEDT_KCD_BANK_SFTCODE;
        tObj.ADDR1 = dataEDT_KCD_BANK_ADDR1;
        tObj.STATUS_CD = dataEDT_KCD_BANK_STATUS_CD.CD_CODE;
        tObj.UPD_USER = dataEDT_KCD_BANK_UPD_USER;
        return tObj;
    };
    ...
    const put_dataEDT_KCD_BANK = (argData) => {
        ...
        editEDT_KCD_BANK_BANK_TYPE1(argData.BANK_TYPE1);
        ...
        editEDT_KCD_BANK_STATUS_CD(argData.STATUS_CD);
        setDataEDT_KCD_BANK_UPD_USER(argData.UPD_USER);

        if (argData.BANK_TYPE1 === "2") {
            // Foreign
    ...
    const onRowClick1TBL_KCD_BANK = (argData0) => {
        ...
        let argTBL_KCD_BANK = argData;
        editTBL_KCD_BANK(argTBL_KCD_BANK);
        setDataTBL_KCD_BANK(argTBL_KCD_BANK);

        search_LIST_3(argData);
    };

    const onRowClickTBL_KCD_BANK = (event) => {
        // Service : NawooAll:mgrQueryTBL_KCD_BANK
    };

    const exportExcelTBL_KCD_BANK = () => {
        import("xlsx").then((xlsx) => {
            const worksheet = xlsx.utils.json_to_sheet(datasTBL_KCD_BANK);
            const workbook = {
            ...
            saveAsExcelFileTBL_KCD_BANK(excelBuffer, "공장목록");
        });
    };

    const saveAsExcelFileTBL_KCD_BANK = (buffer, fileName) => {
    ...
    const editTBL_KCD_VENDOR = (argData) => {
        // put_dataEDT_KCD_VENDOR(argData);
    };
    const onRowClick1TBL_KCD_VENDOR = (argData0) => {
        ...
        editTBL_KCD_VENDOR(argTBL_KCD_VENDOR);
    ...
    const process_DELETE_VENDOR = async () => {
        ...
        if (
            ...
            var tRet = await confirm(
                "GW 상태가 응답대기인것을 삭제 하시겠습니까?<br><br>Do you want to delete the GW status that is waiting for a response?",
            );
            if (tRet);
            else return;
        } else {
            alert("GW 상태가 New, 응답대기, 반려만 삭제가능합니다<br><br>Only GW status of New, waiting for response, and rejected can be deleted.");
            return;
        }
