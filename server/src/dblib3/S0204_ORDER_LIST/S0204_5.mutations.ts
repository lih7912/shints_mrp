...
const moduleMutation_S0204_5 = {
    Mutation: {
        mgrUpdate_S0204_5_END_ORDER: async (_, args, contextValue) => {
            //
...
*/
            ...
            for (tIdx = 0; tIdx < args.datas.length; tIdx++) {
                ...
                var sql0 = `
                    select * from ksv_order_mst 
                    where order_cd = '${tOne.ORDER_CD}'
                `;
                ...
                var sql1 = `
                    select 
                          ...
                          isnull(sample_end_flag , '') as sample_end_flag
                     from kzz_sample_cost 
                    where order_cd = '${tOne.ORDER_CD}'
                `;
                var ret1 = await prisma.$queryRaw(Prisma.raw(sql1));

                var sqlSampleCostFlag =  '';
                if (ret1.length > 0 && 
                    ret1[0].sample_end_flag === '1'  &&
                    ret0[0].FACTORY_CD === 'FC010' &&
                    ret0[0].SAMPLE_FLAG ===  '1')   {
                    sqlSampleCostFlag =  ` sample_cost_flag = '1', `;
                }
        ...
        mgrUpdate_S0204_5_END_ORDER_CANCEL: async (_, args, contextValue) => {
            //
            var tDateNew = new Date();
            tDateNew.setMonth(tDateNew.getMonth() + 1);
            var tZeroDate = '00';
...
*/
        ...
        mgrUpdate_S0204_5_CHANGE_FOB: async (_, args, contextValue) => {
            //
            var tDateNew = new Date();
            tDateNew.setMonth(tDateNew.getMonth() + 1);
            var tZeroDate = '00';
...
*/
            ...
            `;
            var ret0 = await prisma.$queryRaw(Prisma.raw(sql0));
            var tUsdPrice = parseFloat(tInput1.USD_PRICE);
            if (tInput1.CURR_CD === 'USD') tUsdPrice = tInput1.AVR_PRICE;
            else {
                if (ret0.length > 0) {
                    tUsdPrice =
                        parseFloat(tInput1.AVR_PRICE) * ret0[0].USD_RATE;
        ...
        mgrUpdate_S0204_5_UPDATE_DETAIL: async (_, args, contextValue) => {
            ...
            for (tIdx = 0; tIdx < args.datas.length; tIdx++) {
                ...
                tStep1 = nRet0[0].cnt;
                console.log(`1. (step-1) Price 0 check: ${tStep1}`);
                if (nRet0[0].cnt <= 0) {
                    var tSQL1 = `
                        select
                            a.po_cd,
                    ...
                    var tCheckCnt = 0;
                    nRet1.forEach((col1, i1) => {
                        if (col1.check1 === 'not') tCheckCnt += 1;
                    });
                    tStep1 = tCheckCnt;
                    console.log(`1. (step-2) STS-IN Check: ${tStep1}`);
                    /*
              var tSQL1_bak = `
                  select
                      isnull(count(*), 0) as cnt
              ...
              var nRet1 = await prisma.$queryRaw(Prisma.raw(tSQL1));
              tStep1 = nRet1[0].cnt;
              console.log(`1. INPUT:step-1-1: ${tStep1}`);
*/
                }
                if (tStep1 > 0) tEndStatus[0] = '1';
                else tEndStatus[0] = '0';
                console.log(`Write => 1.(Input or Price) : ${tEndStatus[0]}`);
                ...
                console.log(`Write => 2.(CMPT FC_PRICE) : ${tEndStatus[1]}`);
                ...
                console.log(`Write => 3.(SHIP Invoic3) : ${tEndStatus[2]}`);
                ...
                nRet5.forEach((col, i) => {
                    ...
                    tInvoiceTot += col.ship_qty * tPrice;
                });
                if (tShipTot > 0) {
                    console.log(`Invoice Fob: ${tInvoiceTot} / ${tShipTot} `);
                    tInvoiceFob = tInvoiceTot / tShipTot;
                }
                ...
                if (tInvoiceFob.toFixed(2) !== tOrderFob.toFixed(2))
                    tEndStatus[3] = '4';
                else tEndStatus[3] = '0';
                console.log(
                    `Write => 4. FOB:${tEndStatus[3]}:${tInvoiceFob.toFixed(2)} / ${tOrderFob.toFixed(2)}`,
                );


                // 4-1. invoic3, 선적수량관련 (전표 수량 반영. 260703 .Won)
                var sql5 = `
                    select
                        a.sales_price,
                        a.ship_price,
                        ...
                        e.curr_cd ,
                        isnull(e.sample_flag, '') as sample_flag, 
                ...
                nRet5.forEach((col, i) => {
                    ...
                    tSampleFlag = col.sample_flag;

                    console.log(`================> Step4 Check: ${col.docu_no_domestic}, ${col.docu_no_import}, ${tShipQty}, ${tPrice}`);
                ...
                if (tSampleFlag !== '1') {
                    if (tStep4Check > 0) {
                        ...
                        // 일단 막음.  ksv_order_fob 에 값이잘못 들어감.
                        // payment_type이 FOC 인거에 금액이 들어가 있음
                        /*
                        var tDiff = parseFloat(tInvoiceFOB) - parseFloat(tOrderFOB);
                        if (tDiff < -1 || tDiff > 1)
                            tEndStatus[3] = '4';
                        else tEndStatus[3] = '0';
                        console.log(
                             `Write => 4-1. FOB:${tEndStatus[3]}:${tInvoiceFOB} / ${tOrderFOB}`,
                        );
                        */
                    }
                } else {
                ...
                */
                ...
                if (
                    ...
                    var tTotMatlAmt = 0;
                    var tIdx1 = 0;
                    for (tIdx1 = 0; tIdx1 < nRet8.length; tIdx1++) {
                        var col = nRet8[tIdx1];
                        ...
                        var tUsdRate = 0;
                        if (nRet9.length > 0) tUsdRate = nRet9[0].usd_rate;
...
export default moduleMutation_S0204_5;
