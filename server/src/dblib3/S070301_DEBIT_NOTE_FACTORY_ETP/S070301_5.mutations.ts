...
const moduleMutation_S070301_5 = {
    Mutation: {
        mgrInsert_S070301_5_UPDATE_END_TYPE: async (_, args, contextValue) => {
        ...
        mgrInsert_S070301_5_UPDATE_CRDB_DATE: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
        ...
        mgrInsert_S070301_5_UPDATE_CONFIRM: async (_, args, contextValue) => {
            var tRetDate = AFLib.getCurrTime();
            var tRetDate1 = tRetDate.substring(0, 8);
            var tUserInfo = AFLib.getUserInfo(contextValue);
        ...
        // Insert Debit Node
        mgrInsert_S070301_5_INSERT_DEBIT_NOTE_FACTORY_ETP: async (
            _,
            args,
            contextValue,
        ) => {
...
*/
            ...
            var tDebitSeq = 1;
            var tDebitSeqStr = 1;
            var tDebitPrefix1 = `DE${tRetDate.substring(2, 4)}-`;
            var tSQL = `
                SELECT
            ...
            var tNewCreditType = '';
            var tCreditSeq = 1;
            tNewDebitCd = `DE${tYY}-${tDebitSeqStr}`;
            tNewCrdbType = 'E';
      ...
      */
        ...
        // Debit Node Delete
        mgrInsert_S070301_5_CANCEL_DEBIT_NOTE_FACTORY_ETP: async (
            _,
            args,
            contextValue,
        ) => {
      ...
      */
        ...
        // Debit Node End
        mgrInsert_S070301_5_END_DEBIT_NOTE_FACTORY_ETP: async (
            _,
            args,
            contextValue,
        ) => {
            ...
            if (tInput.DEBIT_NO && selectedItem.length == 1) {
                ...
                if (tCrdbObj.STATUS_CD !== '0' && tCrdbObj.STATUS_CD !== '2') {
                    var tRetArray = [];
                    var tObj = {};
                    tObj.CODE = `ERROR:End Debit: 'ERROR: 등록 or 부분End만 End가 가능합니다.'`;
                    tObj.id = 0;
                    tRetArray.push(tObj);
                    return tRetArray;
                }
            ...
            if (selectedItem.length >= 2) {
                for (let item of selectedItem) {
                    ...
                    var tStatusCd = '1';
                    tSumEndAmt += parseFloat(item.CRDB_AMT);
                    if (parseFloat(tSumEndAmt) < parseFloat(tCrdbObj.CRDB_AMT))
                        tStatusCd = '2';
                    let tSQL99 = `
                        update ksv_crdb_mst
        ...
        mgrInsert_S070301_5_CANCEL_END_CREDIT: async (
            _,
            args,
            contextValue,
        ) => {
        ...
        mgrInsert_S070301_5_UPDATE_DEBIT_NOTE_FACTORY_ETP: async (
            _,
            args,
            contextValue,
        ) => {
      ...
      */
        ...
        //  GW (국내자재)
        mgrUpdate_S070301_5_GW_DEBIT_NOTE_FACTORY_ETP: async (
            _,
            args,
            contextValue,
        ) => {
...
export default moduleMutation_S070301_5;
