...
restApi.get('/ha-monitor', async (req, res) => {
    try {
        await prisma.$queryRaw(Prisma.raw('/* ha-monitor-addon */ SELECT 1 AS ok'));
        res.status(200).json({ ok: true });
    } catch (err) {
        console.error('[ha-monitor-addon] health check failed:', err);
...
restApi.all('/del_docuseq/:docu_no/:database', async (req, res) => {
    const tDocuNo = req.params.docu_no;
    const tDatabase = req.params.database;
    let sqlStr = ` 
    select top 1 tp_docu , nm_note
      from neoe.fi_adocu 
    ...
    if (tDatabase === 'shints') {
       ...
       if (tNmNote.includes('삭제요') || tNmNote.includes('AF 테스트')) tCheck = 0; 
       else tCheck = 1; 
    }
    ...
    try {
        if (tTpDocu === 'Y') {
            ...
            if (tCheck === 0) {
                ...
                var tObj = {};
                tObj.MESSAGE = 'ERROR:테스트에서는 리얼전표를 취소할수 없습니다';
                tObj.DOCU_NO = tDocuNo;
                tRetArray.push(tObj);
            }
        }
...
restApi.all('/insert_docu_oversea_product/:user_id', async (req, res) => {
    ...
    for (tIdx = 0; tIdx < tInData.length; tIdx++) {
        ...
        if (typeof col2.row_id !== 'undefined') col2.cd_exch = strNeoeCurrCd;
        col3.cd_exch = strNeoeCurrCd;
        ...
        if (typeof col2.row_id !== 'undefined') col2.rt_exch = strRateBase;
        col3.rt_exch = strRateBase;
...
restApi.all('/insert_docu_domestic_product/:user_id', async (req, res) => {
    ...
    for (tIdx = 0; tIdx < tInData.length; tIdx++) {
        ...
        var col0 = { ...tInData[0].DATA1[0] };
        let sql0 = `
                  select cd_sysdef
                  from neoe.ma_codedtl
                  where cd_field='MA_B000005'
                  and cd_company = '1000'
                  and nm_sysdef = '${col0.nm_mngd4}'
                   `;
         var tRet0 = await prisma.$queryRaw(Prisma.raw(sql0));
         var strNeoeCurrCd = '';
         if (tRet0.length > 0) strNeoeCurrCd = tRet0[0].cd_sysdef;
         col0.cd_exch = strNeoeCurrCd; // col
         console.log(strNeoeCurrCd);
         ...
         let sql1 = `
                  ...
                  from neoe.ma_exchange
                  where curr_sour = '${strNeoeCurrCd}'
                  and   yymmdd = '${col0.dt_acct}'
                   `;
         var tRet1 = await prisma.$queryRaw(Prisma.raw(sql1));
         var strRateBase = '1';
         if (tRet1.length > 0) strRateBase = tRet1[0].rate_base;
         col0.rt_exch = strRateBase; // col
         console.log(strRateBase);
     
         let sql6 = `
                   select nm_mngd
                   from neoe.fi_mngd
                   where cd_mngd = '${col0.AF_A23_CODE}'
                   and cd_mng='a23'
                   and cd_company='1000'
                   `;
         var tRet6 = await prisma.$queryRaw(Prisma.raw(sql6));
         var strA23Name = '';
         if (tRet6.length > 0) strA23Name = tRet6[0].nm_mngd;
         col0.nm_mngd3 = strA23Name;  // col2
         console.log(strA23Name);
     
         let sql7 = `
                   select isnull(max(no_docu),'00000') as max_seq
                   from neoe.fi_adocu
                   where no_docu like 'IS${col0.dt_acct}%'
                   `;
         var tRet7 = await prisma.$queryRaw(Prisma.raw(sql7));
         var tMaxDocuNo = 1;
         if (tRet7[0].max_seq === '00000') {
         ...
         var tZero = '00000';
         var strMaxDocuNo = tZero.substring(0, 5 - String(tMaxDocuNo).length) + String(tMaxDocuNo);
         var strDocuNo = `IS${col0.dt_acct}${strMaxDocuNo}`;
         var strTaxNo = `${strDocuNo}001`;

         var workArray = [];
         tInData[tIdx].DATA1.forEach((col9, i9) => {
            var tObj9 = { ...col9 };
            tObj9.row_id = strDocuNo;
            tObj9.no_docu = strDocuNo;
            tObj9.cd_exch = col0.cd_exch;
            tObj9.rt_exch = col0.rt_exch;
            workArray.push(tObj9);
         });
         tInData[tIdx].DATA2.forEach((col9, i9) => {
            var tObj9 = { ...col9 };
            tObj9.row_id = strDocuNo;
            tObj9.no_docu = strDocuNo;
            tObj9.no_tax  = `${strDocuNo}001`;
            workArray.push(tObj9);
         });
         tInData[tIdx].DATA3.forEach((col9, i9) => {
            var tObj9 = { ...col9 };
            tObj9.row_id = strDocuNo;
            tObj9.no_docu = strDocuNo;
            tObj9.nm_mngd3 = col0.nm_mngd3; 
            workArray.push(tObj9);
         });

         var tSQLArray = [];
         var tIdx2 = 0;
         for (tIdx2 = 0; tIdx2 < workArray.length; tIdx2++) {
             var col = { ...workArray[tIdx2] };
        ...
        // console.log(`Insert SQL Length:[${strDocuNo}/${col0.nm_note}] =>  ${tSQLArray.length}`);
        try {
            await prisma.$transaction(tSQLArray);
            var tObj = {};
            tObj.INVOICE_NO = col0.nm_note;
            tObj.DOCU_NO = strDocuNo;
            tRetArray.push(tObj);
        } catch (e) {
            console.log(`ERROR: insert Docu:${e.message}`);;
...
restApi.all('/insert_docu_import_material/:user_id/:op_kind', async (req, res) => {
    var tInData = [...req.body];
    console.log(tInData);
    console.log(tInData.length);
...
restApi.all('/insert_docu_domestic_material/:user_id/:op_kind', async (req, res) => {
    var tInData = [...req.body];
    // console.log(tInData);
    // console.log(tInData.length);
